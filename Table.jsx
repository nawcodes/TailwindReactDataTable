...
const Table = ({ columns, rowData, actions, onSearch, itemsPerPage = 5 }) => {
    ...
    const filteredData = rowData.filter((item) => {
        ...
        columns.forEach((column) => {
            if (column.key !== "actions") {
                const cellData = column.key.includes(".")
                    ? column.key
                          .split(".")
                ...
                if (cellData) {
                    const cellDataString = String(cellData).toLowerCase();
                    if (cellDataString.includes(searchText.toLowerCase())) {
                        matchFound = true;
                    }
                }
            }
    ...
    const dataWithNumbering = filteredData.map((value, index) => {
                ...
                children: Array.isArray(actions.props.children)
                    ? actions.props.children.map((child, i) => {
                          ...
                          if (
                              ...
                              return (
                                  <Button dataValue={result} {...newAttribute}>
                                      {child.props.children}
                                  </Button>
                              );
                          }
